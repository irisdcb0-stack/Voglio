...
drop policy if exists "public can insert order requests" on public.order_requests;
...
drop policy if exists "public can update service slots remaining spots" on public.service_slots;
create policy "public can update service slots remaining spots"
on public.service_slots
for update
to anon
using (true)
with check (remaining_spots >= 0 and remaining_spots <= capacity);
...
-- Ensure the anonymous role has needed privileges for public access via PostgREST
